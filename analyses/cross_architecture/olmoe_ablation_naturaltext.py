...
def main():
    ...
    ap.add_argument("--out", default="olmoe_ablation_naturaltext.json")
    args = ap.parse_args()
    ...
    # The 4-head induction circuit from synthetic ablation
    induction_circuit = {5: [10], 7: [0], 9: [8], 12: [14]}
    ...
    conditions = [
        ("baseline", {}),
        ("ablate_4_induction_heads (L5H10, L7H0, L9H8, L12H14)", induction_circuit),
        (f"ablate_matched_random_same_layers ({matched_random})", matched_random),
        ("ablate_all_heads_in_those_4_layers (upper bound)", upper_bound),
