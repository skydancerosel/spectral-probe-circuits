...
# Default log-spaced revision set for OLMo-1B-0724-hf
# (range step1000→step1454000, ~2B→3048B tokens)
DEFAULT_REVISIONS = [
    "step1000-tokens2B",
    "step2000-tokens4B",
    "step5000-tokens10B",
    "step11000-tokens23B",
    "step25000-tokens52B",
    "step56000-tokens117B",
    "step126000-tokens264B",
    "step285000-tokens597B",
    "step644000-tokens1350B",
    "step1454000-tokens3048B",  # final
]
...
def main():
    ...
    for ck_idx, rev in enumerate(args.revisions):
        ...
        t0 = time.time()
        try:
            model = OlmoForCausalLM.from_pretrained(args.model, revision=rev, dtype=dtype)
