...
>
> Whole-model BOS-class fractions (fraction of *all heads* classified as first-token at ≥30× selectivity) on synthetic: Pythia 160M 43% / 410M 58% / 1B 54% / OLMoE 1B-7B 68% / OLMo 1B 78%. DCLM data adds ~20pp over Pile at the same scale+architecture; MoE *reduces* BOS by ~10pp vs dense at the same scale+data. When BOS dominance is high (>70%), the all-head capability-specific screen is the robust method for finding capability heads. Universal: **L0 and L1 have zero BOS-classified heads across all five models** — the BOS attractor kicks in from L2 (DCLM) or L4–L6 (Pile) onward.
