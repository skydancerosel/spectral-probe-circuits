...
- **BOS dominance is widespread across decoder-only LMs at 100M+ scale, not unique to OLMoE.** Whole-model BOS-class fractions on synthetic and natural-text:
  ...
  BOS-class fraction here is the fraction of *all heads* classified as first-token at ≥30× selectivity.

- **L0 and L1 have ZERO BOS-classified heads across ALL 5 models** — a universal architectural property. Early layers do diverse general-purpose computation; the BOS attractor kicks in from L2 (DCLM models) or L4–L6 (Pile models).
...
Fraction of *all heads* classified as first-token at ≥30× selectivity, on synthetic and on natural text, across the panel:
...
- ~~Is the BOS attractor MoE-specific, data-specific, or training-config-specific?~~ **Mostly resolved: BOS is broad across decoder-only LMs at 100M+ scale; MoE reduces vs dense; DCLM adds ~20pp vs Pile.** Whole-model BOS fractions span 43% (Pythia 160M dense Pile) → 78% (OLMo 1B dense DCLM). Scale and DCLM data each contribute ~10–20pp to BOS, and MoE *reduces* BOS by ~10pp vs dense at same scale+data. Still open: whether the DCLM effect is data per se or training-recipe (AllenAI pretraining setup); would need a DCLM-trained LM from a different vendor to fully disambiguate.
