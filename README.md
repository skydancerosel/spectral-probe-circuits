...
- **A general claim about retrieval circuits in production LLMs** — but the obvious follow-up has been done. We applied the spectral signal to GPT-2 124M trained on FineWeb-10B (no probe injection) and ran a 6-class capability survey on the top-30 picks: induction, previous-token, duplicate-token, first-token (BOS), self-attention, local. **Of the top-15 picks, all 15 are recognizable capability heads.** At top-30 the precision is still 93% (28 of 30 classified; 2 weakly-content-dependent diffuse heads). Class breakdown across top-30: 14 self-attention, 9 previous-token, 5 induction, 2 unclassified. So the signal generalizes from synthetic to natural-text: not noisily, but *multi-capability* — one signal flagging multiple distinct head classes simultaneously, with mechinterp needed downstream to label which capability each pick implements. Full details in [INDUCTION_HEADS.md](INDUCTION_HEADS.md). The single methodological caveat is that PR-spread doesn't perfectly rank by capability strength: L6H9 has 8,105× previous-token selectivity but ranks only 14 by spread; L7H4 has 184× induction at rank 23. The signal flags real heads but doesn't always rank them by how clean their pattern is.
