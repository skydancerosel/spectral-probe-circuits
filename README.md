...
> **Methodology paper: [`methodology_paper.md`](methodology_paper.md).** A three-step recipe — spectral signal (PR-integral over training) → task-pattern screen (all-head capability-specific selectivity) → causal verification (group ablation vs matched-random) — validated across **6 language models, 2 architecture families (dense transformer, MoE), 3 training pipelines (TS-probe, Pile, DCLM), 2 capability classes (induction, previous-token), and a composed task (IOI)**. The paper includes an explicit scope statement on pattern-vs-task-causal decoupling.
>
> **Companion notes:**
> - [`cross_architecture/developmental_note.md`](cross_architecture/developmental_note.md) — 10-revision mech-interp sweep across Pythia 1B / OLMo 1B / OLMoE 1B-7B. L0/L1 zero-BOS holds at *every* training checkpoint; induction emerges before the BOS attractor in DCLM models by 10–20× in tokens; and the induction circuit reaches its final *size* within 0.3–2% of training but its *membership keeps turning over* — formation-vs-final head overlap ≈ ⅓ (Jaccard 0.29–0.33, robust across 30–100× thresholds), with strong early induction heads decaying out (in OLMoE the two top heads at formation fall 165×/156× → 3×/1×) while others are recruited late. The attention sink, by contrast, *only accretes* — zero head dropouts at any threshold in any model — so turnover is a property of specialized circuits, not the sink, and is broadest in the MoE.
> - [`cross_architecture/ioi_extension_note.md`](cross_architecture/ioi_extension_note.md) — four candidate screens for IOI (prev-token, induction, name-mover, S-Inhibition) across the three 1B models. Each model uses a *different* primary screen; the differential vs matched-random in the same layers is the specificity guarantee. Same task, three mechanistic implementations.
>
> **Cross-architecture extension (May 2026)** to 1B-class models — Pythia 1B (dense Pile), OLMo-1B-0724-hf (dense DCLM), OLMoE-1B-7B-0924 (MoE DCLM). The 3–4 head capability-specific screen at induction-selectivity ≥50× tanks synthetic-induction top-1 from ~5% → 0% across every model; on natural-text contexts the same circuit produces a 5–7× differential effect over matched-random ablations. **L0 and L1 have zero BOS-classified heads across all 5 models** — universal architectural property. Writeup, scripts, and result JSONs in [**cross_architecture/**](cross_architecture/); blog summary in [**cross_architecture/BLOG.md**](cross_architecture/BLOG.md).
>
> Whole-model BOS-class fractions (fraction of *all heads* classified as first-token at ≥30× selectivity): Pythia 160M 43% · 410M 58% · 1B 54% · OLMoE 1B-7B 68% · OLMo 1B 78% on synthetic; 15% · 69% · 68% · 73% · 84% on natural text. MoE *reduces* BOS by ~10pp vs dense at same scale+data; DCLM adds ~20pp over Pile at same scale+architecture.
