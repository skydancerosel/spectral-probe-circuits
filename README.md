...
1. **Why do s271 and s149 recruit late layers (L6/L7) in addition to the shared L0 substrate, while s42 stops at L0?** Plausible candidates: (a) different initial QK matrices favor different layer localizations early in training; (b) the late-layer recruitment is a "second pass" of capacity allocation that depends on stochastic early-training trajectories. We did a quick test of (a) — comparing the per-head SVD of the QK matrix at step 1 across seeds, looking for whether spectral-pick heads have systematically different initial features from non-picks. Result: directional but inconclusive. In 5 of 8 (seed, layer) cells the picks had higher initial top-singular-value or top-1/top-2 gap (z=+0.6 to +1.7), in 2 cells the direction reversed (s271 L7, s256 L6). Suggestive of a "lottery ticket" interpretation but noisy at n=4 seeds × 16 heads/layer. A clean test would need a larger model and/or more seeds. Why s271 and s149 pick *different specific* late-layer heads is a deeper version of the same question.
