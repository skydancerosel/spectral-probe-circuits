...
parameter-side. The Spectral Emergence Dynamics paper \citep{xu2026sed}
tracks the singular-value geometry of attention weight matrices through
training, and reports phase-transition-like changes in the spectral
distribution at training-token counts that coincide, in their panel,
with capability-emergence events identified by downstream evaluation.
The Local Curvature Hypothesis paper \citep{xu2026lch} examines the
local loss-Hessian during training and finds curvature signatures at
the same training-token counts. The picture both papers develop is that
