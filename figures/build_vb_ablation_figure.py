"""Figure 5: variable-binding ablation across three 1B configurations.

Two panels: left = Δtop-1, right = Δlogit_diff.
Same design as Figure 3 (GT) and Figure 4 (Successor).
"""
...
CONDITIONS = [
    ("top-5 VB screen",      "ablate_top",                    "#8c2d04"),
...
def get_deltas():
    ...
    for label, short in MODELS:
        with open(WORKTREE / f"vb/{short}_vb_ablation.json") as f:
            d = json.load(f)
        base = d["conditions"][0]
        b1 = base["top1"]
        bl = base["logit_diff_mean"]
        out[short] = {}
        for cond_label, key, _ in CONDITIONS:
            ...
            if c is None:
                ...
                continue
            d_top1 = (c["top1"] - b1) * 100
            d_logit = c["logit_diff_mean"] - bl
...
ax1.set_title("A   Δ top-1 (correct-city argmax)  on ablation",
...
for ci, (cond_label, _, color) in enumerate(CONDITIONS):
    ...
    for x, y in zip(xs, ys):
        if y is None: continue
        if y >= 0:
            va, offset_y = "bottom", 0.04
        else:
            va, offset_y = "top", -0.04
...
ax2.set_ylabel("Δ logit_diff  (target − distractor)")
ax2.set_title("B   Δ logit_diff (target − distractor city)  on ablation",
              loc="left", fontsize=11.0, fontweight="bold", pad=8)
ax2.set_ylim(-1.2, 1.3)
...
fig.suptitle("Variable binding: an interferer screen in Pythia, prev-token primary in OLMo/OLMoE",
             fontsize=12.0, fontweight="bold", y=1.10)
plt.tight_layout()

out_png = HERE / "vb_ablation_figure.png"
out_pdf = HERE / "vb_ablation_figure.pdf"
...
for label, short in MODELS:
    ...
    for cond_label, _, _ in CONDITIONS:
        ...
        print(f"    {cond_label:<24}  Δtop1={dt:+6.2f}pp   Δlogit={dl:+6.2f}")
